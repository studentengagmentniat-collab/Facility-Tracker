...
export default function Tracker({ user, profile, onLogout }) {
  ...
  async function submitNew(form) {
    ...
    if (data) {
      if (form.remark) await supabase.from('remarks').insert({ request_id: data.id, text: form.remark, added_by: profile?.name || 'Requester', added_by_email: user.email });
    ...
    <div style={s}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '8px' }}>
        <div>
          ...
          <div style={{ fontSize: '12px', color: '#888' }}>Welcome, {profile?.name || user.email} ({profile?.role || 'user'})</div>
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          <button style={btn('#1a73e8')} onClick={() => setModal('new')}>+ New Request</button>
      ...
      {view === 'updates' ? (
        ...
        <div>
          ...
          {filtered.map(r => {
            ...
            const canAdv = r.status !== 'rejected' && idx < STAGES.length - 1;
            const canRej = r.status !== 'rejected' && r.status !== 'received';
        ...
        <div style={box}>
          ...
          <div><label style={{ fontSize: '12px', color: '#666' }}>Remark by</label>
            <select style={inp} value={remarkBy} onChange={e => setRemarkBy(e.target.value)}>
              <option>{profile?.name || 'Requester'}</option>
              <option>Manager</option><option>Facility Team</option><option>Finance</option>
            </select></div>
